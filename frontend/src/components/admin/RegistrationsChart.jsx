        ...
        <div className={`rounded-2xl p-6 shadow-sm border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100/80'}`}>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className={`text-sm mt-0.5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Daily registration activity</p>
                </div>
            ...
            {hasData ? (
                <ResponsiveContainer width="100%" height={260}>
                    <AreaChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <XAxis dataKey="day" tick={{ fontSize: 12, fill: isDark ? '#9ca3af' : '#94a3b8' }} axisLine={false} tickLine={false} />
                        <YAxis tick={{ fontSize: 12, fill: isDark ? '#9ca3af' : '#94a3b8' }} axisLine={false} tickLine={false} allowDecimals={false} />
                        <Tooltip
                            contentStyle={{
                        ...
                        <Area
                            ...
                            dot={{ r: 4, fill: '#6366f1', stroke: isDark ? '#1f2937' : '#fff', strokeWidth: 2 }}
                            activeDot={{ r: 6, fill: '#6366f1', stroke: isDark ? '#1f2937' : '#fff', strokeWidth: 2 }}
                        />
                    </AreaChart>
                </ResponsiveContainer>
            ) : (
                <div className={`flex flex-col items-center justify-center h-[260px] ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
